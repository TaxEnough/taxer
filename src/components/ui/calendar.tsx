...
import { DayPicker, CaptionProps, MonthChangeEventHandler } from "react-day-picker"
...
// Custom caption component to add year selector
function CustomCaption({ displayMonth, onMonthChange }: CaptionProps) {
  const year = getYear(displayMonth)
  const handleYearChange = (selectedYear: string) => {
    const newMonth = setYear(displayMonth, parseInt(selectedYear))
  ...
  return (
    <div className="flex items-center justify-center gap-1 relative">
      <Select defaultValue={year.toString()} onValueChange={handleYearChange}>
        <SelectTrigger className="w-[4.5rem] h-7 text-xs border-none focus:ring-0 p-1 mx-1">
      ...
      <span className="text-sm font-medium">
        {format(displayMonth, "MMMM", { locale: tr })}
      </span>
    </div>
  )
}
